...
- **API public/protected phát hiện được:**
  ...
  - **Dòng 23 — `public static byte[] encode(DataAssetBundle bundle) throws IOException {`**: Mã hóa năm graphics block, task routes, EXP, progression và effect-template tail.
...
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 7 — `public final class DataAssetSeedArtifact {`**: DATA candidate bất biến gồm payload đã kiểm định và manifest xác định.
  - **Dòng 19 — `public byte[] payload() {`**: Trả defensive copy để caller không sửa candidate đã khóa checksum.
  - **Dòng 24 — `public DataAssetSeedManifest manifest() {`**: Trả manifest đã dùng để tự kiểm định candidate.
  - **Dòng 29 — `public String manifestText() {`**: Trả manifest UTF-8 key=value với thứ tự dòng cố định.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
...
- **Vai trò tóm tắt:** Sinh DATA candidate xác định và tự kiểm định trước khi trả artifact.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 7 — `public final class DataAssetSeedArtifactGenerator {`**: Sinh DATA candidate xác định và tự kiểm định trước khi trả artifact.
  - **Dòng 14 — `public static DataAssetSeedArtifact generate(DataAssetBundle bundle) {`**: Encode bundle, khóa metadata/checksum rồi validate lại cùng candidate.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.

### `com.nsocry.assets.DataAssetSeedManifest`

- **Source:** `src/main/java/com/nsocry/assets/DataAssetSeedManifest.java`
- **Vai trò tóm tắt:** Manifest xác định khóa metadata và SHA-256 của một DATA candidate.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 8 — `public record DataAssetSeedManifest(`**: Manifest xác định khóa metadata và SHA-256 của một DATA candidate.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.

### `com.nsocry.assets.DataAssetSeedManifestParser`

- **Source:** `src/main/java/com/nsocry/assets/DataAssetSeedManifestParser.java`
- **Vai trò tóm tắt:** Parse manifest DATA seed v1 canonical, từ chối field thừa, thiếu hoặc đổi thứ tự.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 6 — `public final class DataAssetSeedManifestParser {`**: Parse manifest DATA seed v1 canonical, từ chối field thừa, thiếu hoặc đổi thứ tự.
  - **Dòng 13 — `public static DataAssetSeedManifest parse(String text) {`**: Parse đúng sáu dòng key=value và yêu cầu newline LF kết thúc file.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.

### `com.nsocry.assets.DataAssetSeedValidationResult`

- **Source:** `src/main/java/com/nsocry/assets/DataAssetSeedValidationResult.java`
- **Vai trò tóm tắt:** Metadata DATA đã decode, encode lại và đối chiếu thành công với manifest.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 4 — `public record DataAssetSeedValidationResult(`**: Metadata DATA đã decode, encode lại và đối chiếu thành công với manifest.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.

### `com.nsocry.assets.DataAssetSeedValidator`

- **Source:** `src/main/java/com/nsocry/assets/DataAssetSeedValidator.java`
- **Vai trò tóm tắt:** Kiểm định DATA bundle bằng cách encode lại và đối chiếu toàn bộ manifest candidate.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 10 — `public final class DataAssetSeedValidator {`**: Kiểm định DATA bundle bằng cách encode lại và đối chiếu toàn bộ manifest candidate.
  - **Dòng 15 — `public static DataAssetSeedValidationResult validate(`**: Từ chối mọi khác biệt version/count/length/checksum so với đúng bundle được cung cấp.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
...
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 14 — `public final class ReferenceDataAssetConverter {`**: Ghép dump DATA authoritative và progression đã truy vết thành bundle client V7 hoàn chỉnh.
  - **Dòng 30 — `public static DataAssetBundle convert(`**: Tạo bundle bất biến mà không mở database hoặc publish runtime.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
...
- **API public/protected phát hiện được:**
  ...
  - **Dòng 26 — `public static DataDumpInventoryReport parse(String dump) {`**: Parse đúng tám statement, kiểm tra wire-boundary và trả count phục vụ converter.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
...
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 9 — `public final class ReferenceGameDataProgressionParser {`**: Đọc đúng mười mảng progression từ source GameData reference mà không compile source legacy.
  - **Dòng 27 — `public static EnumMap<ProgressionTable, int[]> parse(String javaSource) {`**: Parse đủ mười declaration int[] theo mapping cố định và trả defensive arrays.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
...
- **Vai trò tóm tắt:** Đọc lại và kiểm định DATA archive offline, không mutation database/runtime/startup.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 8 — `public final class DataAssetSeedArchiveDryRunCommand {`**: Đọc lại và kiểm định DATA archive offline, không mutation database/runtime/startup.
  - **Dòng 13 — `public static void main(String[] args) throws Exception {`**: Entry CLI yêu cầu đúng một archive path và in metadata đã xác minh.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.

### `com.nsocry.bootstrap.DataAssetSeedConvertCommand`

- **Source:** `src/main/java/com/nsocry/bootstrap/DataAssetSeedConvertCommand.java`
- **Vai trò tóm tắt:** Tạo DATA archive từ cùng input authoritative của candidate dry-run đã khóa.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 10 — `public final class DataAssetSeedConvertCommand {`**: Tạo DATA archive từ cùng input authoritative của candidate dry-run đã khóa.
  - **Dòng 15 — `public static void main(String[] args) throws Exception {`**: Entry CLI yêu cầu đúng một DATA properties path.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
...
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 18 — `public final class DataAssetSeedDryRunCommand {`**: Dry-run DATA candidate từ dump và GameData authoritative; không ghi database/runtime/file.
  - **Dòng 27 — `public static void main(String[] args) throws Exception {`**: Entry CLI yêu cầu đúng một đường dẫn properties chứa toàn bộ input explicit.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
...
- **Vai trò tóm tắt:** Xuất và đọc lại archive DATA seed offline; không truy cập database/runtime.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 25 — `public final class DataAssetSeedArchiveService {`**: Xuất và đọc lại archive DATA seed offline; không truy cập database/runtime.
  - **Dòng 32 — `public void export(DataAssetSeedArtifact artifact, Path target) throws IOException {`**: Ghi candidate qua file tạm và atomic move; không ghi đè archive tồn tại.
  - **Dòng 53 — `public DataAssetSeedValidationResult dryRun(Path archive) throws IOException {`**: Dry-run archive và trả metadata sau decode/encode/checksum validation.
  - **Dòng 58 — `public ValidatedDataAssetSeedArchive readValidated(Path archive) throws IOException {`**: Đọc archive fail-closed, không mở JDBC và không publish runtime snapshot.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
...
### `com.nsocry.operations.ValidatedDataAssetSeedArchive`

- **Source:** `src/main/java/com/nsocry/operations/ValidatedDataAssetSeedArchive.java`
- **Vai trò tóm tắt:** Nội dung archive DATA đã qua parse, decode và kiểm định checksum.
- **Trạng thái:** `IMPLEMENTED`
- **API public/protected phát hiện được:**
  - **Dòng 8 — `public final class ValidatedDataAssetSeedArchive {`**: Nội dung archive DATA đã qua parse, decode và kiểm định checksum.
  - **Dòng 24 — `public byte[] payload() {`**: Trả defensive copy để persistence tương lai không sửa candidate đã xác minh.
  - **Dòng 29 — `public String manifestText() {`**: Trả nguyên manifest canonical UTF-8 đã kiểm định.
  - **Dòng 34 — `public DataAssetSeedValidationResult validation() {`**: Trả metadata đã khớp payload và manifest.
- **Khi sửa:** kiểm tra caller bằng `rg`, cập nhật test + manual module + STATUS/WORKLOG; không đổi contract LOCKED nếu thiếu ADR.
